...
def curate_molecule(smiles_with_ids):
    smiles, identifier = smiles_with_ids
    standardized_smiles = standardize_molecule(smiles)
    if standardized_smiles:
        inchikey = get_inchikey(standardized_smiles)
        return standardized_smiles, identifier, inchikey
...
def curate_molecules(smiles_with_ids):
    ...
    batch_size = 500  # Process molecules in batches of 500
    batches = [smiles_with_ids[i:i + batch_size] for i in range(0, len(smiles_with_ids), batch_size)]
    with Pool(processes=cpu_count()) as pool:
        for batch in tqdm(batches, desc="Standardizing molecules", dynamic_ncols=True):
            results = pool.map(curate_molecule, batch)
            for standardized_smiles, identifier, inchikey in results:
                if standardized_smiles and inchikey and inchikey not in seen_inchikeys:
                    seen_inchikeys.add(inchikey)
                    curated_smiles.append((standardized_smiles, identifier))
                elif not inchikey:
                    failed_molecules.append((standardized_smiles, identifier, inchikey))
                else:
                    duplicate_molecules.append((standardized_smiles, identifier, inchikey))
    return curated_smiles, len(failed_molecules), len(duplicate_molecules), failed_molecules, duplicate_molecules
...
def process_files(file_paths):
    for file_path in file_paths:
        ...
        curated_smiles, failed_count, duplicate_count, failed_molecules, duplicate_molecules = curate_molecules(
            smiles_with_ids)
        ...
        print(f"Failed molecules written to {failed_output_file}\n")
