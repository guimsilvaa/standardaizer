...
def strip_cxsmiles(smiles):
    if '|' in smiles:
        smiles = smiles.split('|', 1)[0].strip()
    return smiles

def strip_cxsmiles_identifier(identifier):
    if not identifier:
        return identifier
    identifier = re.sub(r'\|[^|]*\|', '', identifier) # remove CXSMILES blocks like |c:8,t:15|
    identifier = ' '.join(identifier.split()) # normalize whitespace: collapse multiple spaces and strip ends
    return identifier

def standardize_molecule(smiles):
    try:
        smiles = strip_cxsmiles(smiles)  # also clean input, just in case
        mol = Chem.MolFromSmiles(smiles)
        if mol is None:
            return None
        mol = molvs_standardizer.standardize(mol)
        standardized_smiles = Chem.MolToSmiles(mol)
        standardized_smiles = strip_cxsmiles(standardized_smiles)
        fragments = standardized_smiles.split('.')
        if len(fragments) > 1:
            standardized_smiles = max(fragments, key=len)
        if not contains_only_allowed_atoms(standardized_smiles):
            return None
    ...
    for file_path in file_paths:
        ...
        with open(output_file, 'w') as file:
            for smiles, identifier in curated_smiles:
                clean_identifier = strip_cxsmiles_identifier(identifier)
                file.write(f"{smiles}\t{clean_identifier.strip()}\n")
